...
class TestAutoFetchModuleContent:
    ...
    def test_auto_fetch_uses_wikipedia_by_default(self, mock_fetcher_class, sample_module, tmp_path):
        ...
        mock_fetcher = Mock()
        mock_fetcher.search_and_fetch_multiple_sources.return_value = [Path("test.md")]
        ...
        # The sources are reached through the combined search, with Wikipedia enabled
        mock_fetcher.search_and_fetch_multiple_sources.assert_called_once()
        assert mock_fetcher.search_and_fetch_multiple_sources.call_args.kwargs["use_wikipedia"] is True
        assert len(result) > 0
    ...
    def test_auto_fetch_fallback_to_synthetic(self, mock_fetcher_class, sample_module, tmp_path):
        ...
        mock_fetcher = Mock()
        mock_fetcher.search_and_fetch_multiple_sources.return_value = []  # every source fails
        ...
        # Should try the sources first, then fall back
        mock_fetcher.search_and_fetch_multiple_sources.assert_called_once()
    ...
    def test_auto_fetch_respects_disable_flags(self, mock_fetcher_class, sample_module, tmp_path):
        ...
        mock_fetcher = Mock()
        mock_fetcher.search_and_fetch_multiple_sources.return_value = []
        mock_fetcher_class.return_value = mock_fetcher
        ...
        # Wikipedia is disabled where the sources are selected, and no fallback runs
        assert mock_fetcher.search_and_fetch_multiple_sources.call_args.kwargs["use_wikipedia"] is False
